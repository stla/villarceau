...
myTriplets phi = map (tripletOnCircle phi) theta_
  where
    theta_ = [2 * pi * frac i nCircles | i <- [0 .. nCircles-1]]
...
display context alpha = do
  ...
  forM_ (zip (tmatsAndRadii phi) colors) $ \(tmatAndRadius, colour) ->
    preservingMatrix $ do
      m <- newMatrix RowMajor (fst tmatAndRadius) :: IO (GLmatrix GLfloat)
      multMatrix m
      materialDiffuse Front $= colour
      renderObject Solid $ Torus 0.1 (snd tmatAndRadius) 60 60
  swapBuffers
...
keyboard :: IORef GLfloat -> IORef GLfloat -> IORef GLfloat -- rotations
         ...
         -> KeyboardCallback
keyboard rot1 rot2 rot3 zoom anim colors c _ = do
  case c of
    ...
    'l' -> zoom $~! subtract 1
    'k' -> do
      kolors <- replicateM nCircles pickColor
...
main = do
  ...
  rot3 <- newIORef 0.0
  zoom <- newIORef 0.0
  colors <- replicateM nCircles pickColor
  ...
  keyboardCallback $= Just (keyboard rot1 rot2 rot3 zoom anim colors')
